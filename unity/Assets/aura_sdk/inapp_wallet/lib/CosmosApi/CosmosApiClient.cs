...
namespace CosmosApi
{
    public class CosmosApiClient : ICosmosApiClient
    {
        ...
        public override HttpClient GetHttpClient() { 
            return _flurlClient?.Value.HttpClient ?? throw new ObjectDisposedException(nameof(CosmosApiClient));
        }

        public override ICryptoService GetCryptoService(){
            return _settings.CryptoService;
        } 
            ...
            var client = new FlurlClient()
                .Configure(s =>
                {
                    ...
                    if (_settings.OnError != null)
                    {
                        s.OnError = call =>
                        {
                            var error = new Error(call.Request, call.Response, call.StartedUtc, call.EndedUtc.Value, call.Exception.WrapException(), call.ExceptionHandled);
