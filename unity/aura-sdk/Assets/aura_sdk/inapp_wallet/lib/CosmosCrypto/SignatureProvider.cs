...
namespace AuraSDK
{
    public class SignatureProvider
    {
        ...
        static void TestSign(){
            ...
            string expectedSig = "86ab2fdb77c813119b7f90776e2ce1a7da4cdcc75fd0fff3bc0767fe30c160d340e53c8a39d5961efe8f54b91c6e009fece3463f5faae90dbb5c90178f2e9e3c";

            byte[] sigBytes = SignSecp256k1(signDoc, key);
            string actualSig = sigBytes.ToHexString();
        ...
        // [UnityEngine.RuntimeInitializeOnLoadMethod]
        static void TestSignArbitrary(){
            string mnemonic = "endless ten raise noble cream arm run hamster dress way tenant cram";
            string message = "I love Aura";
            string expectedSig = "yN6qzkP7gz65cbQL1QdTPRMm/trB/9whUP9MZk3cQL5aW/9+dEhh8SUtc6Xsyc3M+bnv/B/QteQmz1xZ69zsIw==";

            var wallet = InAppWallet.RestoreHDWallet(mnemonic);
            string actualSig = wallet.SignArbitrary(message.ToByteArrayUTF8().ToBase64String());

            if (actualSig.Equals(expectedSig))
                Logging.Verbose("SignArbitrary completed successfully");
            else 
                Logging.Verbose("SignArbitrary failed");
        }
    }
}
